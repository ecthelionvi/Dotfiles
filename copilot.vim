...
function! s:MapTab() abort
  ...
  let tab_map = maparg('<M-Tab>', 'i', 0, 1)
  if !has_key(tab_map, 'rhs')
    imap <script><silent><nowait><expr> <M-Tab> copilot#Accept()
  elseif tab_map.rhs !~# 'copilot'
    ...
    if get(tab_map, 'noremap') || get(tab_map, 'script') || mapcheck('<Left>', 'i') || mapcheck('<Del>', 'i')
      exe 'imap <script><silent><nowait><expr> <M-Tab> copilot#Accept(' . tab_fallback . ')'
    else
      exe 'imap <silent><nowait><expr>         <M-Tab> copilot#Accept(' . tab_fallback . ')'
    endif
  endif
endfunction
