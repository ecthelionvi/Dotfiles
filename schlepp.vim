...
"}}}
"{{{  Mappings
noremap <silent><unique> <script> <Plug>SchleppUp <SID>SchleppUp
noremap <silent><unique> <script> <Plug>SchleppDown <SID>SchleppDown
noremap <silent><unique> <script> <Plug>SchleppLeft <SID>SchleppLeft
noremap <silent><unique> <script> <Plug>SchleppRight <SID>SchleppRight

noremap <silent><SID>SchleppUp    :call <SID>Schlepp("Up")<CR>
noremap <silent><SID>SchleppDown  :call <SID>Schlepp("Down")<CR>
noremap <silent><SID>SchleppLeft  :call <SID>Schlepp("Left")<CR>
noremap <silent><SID>SchleppRight :call <SID>Schlepp("Right")<CR>

"Reindent Mappings
"These are only done on VisualLine Mode
noremap <silent><unique> <script> <Plug>SchleppIndentUp       <SID>SchleppIndentUp
noremap <silent><unique> <script> <Plug>SchleppIndentDown     <SID>SchleppIndentDown
noremap <silent><unique> <script> <Plug>SchleppToggleReindent <SID>SchleppToggleReindent

noremap <silent><SID>SchleppIndentUp       :call <SID>Schlepp("Up", 1)<CR>
noremap <silent><SID>SchleppIndentDown     :call <SID>Schlepp("Down", 1)<CR>
noremap <silent><SID>SchleppToggleReindent :call <SID>SchleppToggleReindent()<CR>
"}}}
"{{{ s:Schlepp(dir, ...) range
...
"  Logic for moving text selected with visual line mode
    ...
    if a:dir ==? 'up' "{{{ Up
        if l:fline == 1 "First lines of file, move everything else down
            ...
            execute "silent! normal! :'<,'>m'<-2\<CR>" . l:reindent_cmd . 'gv'
        endif "}}}
    elseif a:dir ==? 'down' "{{{ Down
        if l:lline == line('$') "Moving down past EOF
            ...
            execute "silent! normal! :'<,'>m'>+1\<CR>" . l:reindent_cmd . 'gv'
        endif "}}}
...
"  line
    ...
    try
        ...
        if a:dir ==? 'up' "{{{ Up
            ...
            silent! normal! gvxkPgvkoko
            "}}}
        elseif a:dir ==? 'down' "{{{ Down
            if l:lline == line('$') "Moving down past EOF
                call append(line('$'), '')
            endif
            silent! normal! gvxjPgvjojo
            "}}}
        elseif a:dir ==? 'right' "{{{ Right
            silent! normal! gvxpgvlolo
            "}}}
        elseif a:dir ==? 'left' "{{{ Left
            if l:left_col == 1
                execute "silent! normal! gvA \<esc>"
                if g:Schlepp#allowSquishingBlock || match(getline(l:fline, l:lline), '^[^ \t]') == -1
                    for l:linenum in range(l:fline, l:lline)
                        if match(getline(l:linenum), "^[ \t]") != -1
                            call setline(l:linenum, substitute(getline(l:linenum), "^\\s", '', ''))
                            execute 'silent! normal! :' . l:linenum . "\<cr>" . l:right_col . "|a \<esc>"
                        endif
                    endfor
                endif
                call s:ResetSelection()
            else
                silent! normal! gvxhPgvhoho
            endif
        endif "}}}
...
""}}}
"{{{ Mappings
noremap <silent><unique> <script> <Plug>SchleppDupUp <SID>SchleppDupUp
noremap <silent><unique> <script> <Plug>SchleppDupDown <SID>SchleppDupDown
noremap <silent><unique> <script> <Plug>SchleppDupLeft <SID>SchleppDupLeft
noremap <silent><unique> <script> <Plug>SchleppDupRight <SID>SchleppDupRight
noremap <silent><unique> <script> <Plug>SchleppDup <SID>SchleppDup

noremap <silent><SID>SchleppDupUp    :call <SID>SchleppDup("Up")<CR>
noremap <silent><SID>SchleppDupDown  :call <SID>SchleppDup("Down")<CR>
noremap <silent><SID>SchleppDupLeft  :call <SID>SchleppDup("Left")<CR>
noremap <silent><SID>SchleppDupRight :call <SID>SchleppDup("Right")<CR>
noremap <silent><SID>SchleppDup      :call <SID>SchleppDup()<CR>
"}}}
"{{{ s:SchleppDup(...) range
...
"  Logic for duplicating line selections
    ...
    execute 'silent! normal! gvyP' . l:reselect
endfunction "}}}
...
"  Logic for duplicating block selections
    ...
    try
        ...
        if a:dir ==? 'up'
            ...
            execute 'silent! normal! gvy' . l:set_cursor . 'Pgv'
        ...
        elseif a:dir ==? 'down'
            ...
            execute "silent! normal! gvy'>j" . l:left_col . '|Pgv'
        elseif a:dir ==? 'left'
            if l:numcols > 0
                execute 'silent! normal! gvyP' . l:numcols . "l\<C-v>" . (l:numcols + (&selection ==# 'exclusive')) . 'l' . (l:numlines - 1) . 'jo'
            else
                execute "silent! normal! gvyP\<C-v>"  . (l:numlines - 1) . 'jo'
            endif
        elseif a:dir ==? 'right'
            silent! normal! gvyPgv
        else
            call s:ResetSelection()
        endif
